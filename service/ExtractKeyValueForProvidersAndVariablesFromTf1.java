    ...
    public static void main(String[] args) {
        try{    
            ...
            for (File file : listOfFiles) {
              ...
              // "(?:(type)\s*+\=\s*+(\w+)|(default)\s*+\=\s*+((?:(?:\{(?:(?:[^\{\}]*+)(?:\{[^\{\}]*+(?:\{[^\{\}]*+\})*[^\{\}]*+\})*(?:[^\{\}]*+))*\})|(?:\[(?:(?:[^\[\]]*+)(?:\[[^\[\]]*+(?:\[[^\[\]]*+\])*[^\[\]]*+\])*(?:[^\[\]]*+))*\])))|(?:\h*+(?<!\#\h)(?<!\w)(\w+)\s+\=\s+\"?([\/\w\-\$\.\=\ \>\<\#\:\\\[\]]++)\"?))" flags=gis , regex101 get key-value
              
              final Pattern extractProviderVariablePattern = Pattern.compile( "(variable|provider)[\\\"\\:\\s\\{]+([\\w\\-]++)(?:[\\\"\\ ]++)(?:\\{).*?(?:\\})(?=(?:\\s+[\\w\\#]|$))(?![^\\{]*= *\\{)(?![^\\{]*= *[\\w\\$\\\"])"
              , Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
              
              final Pattern extractKeyValuePattern = Pattern.compile( 	"(?:(type)\\s*+\\=\\s*+(\\w+)|(default)\\s*+\\=\\s*+((?:(?:\\{(?:(?:[^\\{\\}]*+)(?:\\{[^\\{\\}]*+(?:\\{[^\\{\\}]*+\\})*[^\\{\\}]*+\\})*(?:[^\\{\\}]*+))*\\})|(?:\\[(?:(?:[^\\[\\]]*+)(?:\\[[^\\[\\]]*+(?:\\[[^\\[\\]]*+\\])*[^\\[\\]]*+\\])*(?:[^\\[\\]]*+))*\\])))|(?:\\h*+(?<!\\#\\h)(?<!\\w)(\\w+)\\s+\\=\\s+\\\"?([\\/\\w\\-\\$\\.\\=\\ \\>\\<\\#\\:\\\\\\[\\]]++)\\\"?))"
              , Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
